...
if ! command -v jq >/dev/null 2>&1; then
    ...
    if command -v apt-get >/dev/null 2>&1; then
        ...
        set +e
        # Use --allow-unauthenticated to bypass GPG signature issues during early build stages
        apt-get update -o Acquire::TempDir="${APT_TMP_ALT:-/opt/apt-temp}" && apt-get install -y -o Acquire::TempDir="${APT_TMP_ALT:-/opt/apt-temp}" jq
        apt_status=$?
        set -e
