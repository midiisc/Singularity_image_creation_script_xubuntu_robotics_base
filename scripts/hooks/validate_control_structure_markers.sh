...
check_if_fi_markers() {
    ...
    while IFS= read -r if_line; do
        local if_num=$(echo "$if_line" | cut -d: -f1)
        local if_content=$(echo "$if_line" | cut -d: -f2-)
        
        # Extract condition/keyword for marker
        local condition=$(echo "$if_content" | sed -E 's/^[[:space:]]*if[[:space:]]+\[\[?[[:space:]]*(.*)[[:space:]]*\]\]?[[:space:]]*;?[[:space:]]*then.*$/\1/' | head -c 40)
        
        # Find matching 'fi' (simplified - looks for next unindented fi)
        local fi_num=$(awk -v start="$if_num" '
            NR > start {
                if (/^[[:space:]]*fi[[:space:]]*$/) {
                    print NR
                    exit
        ...
        if [ -n "$fi_num" ] && [ "$fi_num" -gt "$if_num" ]; then
            ...
            local prev_line=$(sed -n "$((fi_num - 1))p" "$file")
...
check_for_done_markers() {
    ...
    while IFS= read -r for_line; do
        local for_num=$(echo "$for_line" | cut -d: -f1)
        local for_content=$(echo "$for_line" | cut -d: -f2-)
        
        # Extract loop variable/range for marker
        local loop_var=$(echo "$for_content" | sed -E 's/^[[:space:]]*for[[:space:]]+([a-zA-Z_][a-zA-Z0-9_]*).*$/\1/' | head -c 30)
        
        # Find matching 'done'
        local done_num=$(awk -v start="$for_num" '
            NR > start {
                if (/^[[:space:]]*done[[:space:]]*$/) {
                    print NR
                    exit
        ...
        if [ -n "$done_num" ] && [ "$done_num" -gt "$for_num" ]; then
            ...
            local prev_line=$(sed -n "$((done_num - 1))p" "$file")
...
check_while_done_markers() {
    ...
    while IFS= read -r while_line; do
        local while_num=$(echo "$while_line" | cut -d: -f1)
        local while_content=$(echo "$while_line" | cut -d: -f2-)
        
        # Extract condition for marker
        local condition=$(echo "$while_content" | sed -E 's/^[[:space:]]*while[[:space:]]+\[\[?[[:space:]]*(.*)[[:space:]]*\]\]?.*$/\1/' | head -c 40)
        
        # Find matching 'done'
        local done_num=$(awk -v start="$while_num" '
            NR > start {
                if (/^[[:space:]]*done[[:space:]]*$/) {
                    print NR
                    exit
...
check_case_esac_markers() {
    ...
    while IFS= read -r case_line; do
        local case_num=$(echo "$case_line" | cut -d: -f1)
        local case_content=$(echo "$case_line" | cut -d: -f2-)
        
        # Extract case variable/pattern for marker
        local case_var=$(echo "$case_content" | sed -E 's/^[[:space:]]*case[[:space:]]+([^[:space:]]+).*$/\1/' | head -c 30)
        
        # Find matching 'esac'
        local esac_num=$(awk -v start="$case_num" '
            NR > start {
                if (/^[[:space:]]*esac[[:space:]]*$/) {
                    print NR
                    exit
        ...
        if [ -n "$esac_num" ] && [ "$esac_num" -gt "$case_num" ]; then
            ...
            local prev_line=$(sed -n "$((esac_num - 1))p" "$file")
